...
%
% Inputs:	p   number of filters in filterbank过滤器组中有p个过滤器
%		n   length of fft快速傅里叶变化的长度
%		fs  sample rate in Hz采样率
%		fl  low end of the lowest filter as a fraction of fs (default = 0)
%		fh  high end of highest filter as a fraction of fs (default = 0.5)
%		w   any sensible combination of the following:
%		      't'  triangular shaped filters in mel domain (default)三角形
%		      'n'  hanning shaped filters in mel domain汉宁
%		      'm'  hamming shaped filters in mel domain汉明
%
%		      'z'  highest and lowest filters taper down to zero
%		      (default)最高到最低
%		      'y'  lowest filter remains at 1 down to 0 frequency
%		      and最低滤波器保持到1-0频率和
%			   highest filter remains at 1 up to nyquist
%			   freqency最高滤波器保持在1到奈奎斯特频率
%
...
%输出:x一个稀疏矩阵，包含滤波器组振幅
%如果x是唯一的输出参数，那么size(x)=[p,1+floor(n/2)]否则大小(x) = (p, mx-mn + 1)
% mn是非零系数的最低fft
% mx非零系数的最高fft
