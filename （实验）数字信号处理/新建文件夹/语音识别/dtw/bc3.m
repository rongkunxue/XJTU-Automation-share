...
%  frameinc=ceil(2/3*framelenth);
frameoverlaped=framelenth-frameinc;
 Tezheng1=[];%存能量
Tezheng2=[];%存过零率
zhenshu=[];
lable=zeros(1000,1);   
...
train_label=zeros(LL*100,1);% 列向量
practise_label=zeros(LL_dad*100,1);

train_data0=zeros(LL*100,fpsmid);% 矩阵，每行一个样本，每列一个特征
practise_data0=zeros(LL_dad*100,fpsmid);
train_data1=zeros(LL*100,fpsmid);
practise_data1=zeros(LL_dad*100,fpsmid);
k1=1;
k2=1;
for i=1:1:1000
    l=mod(i,200);
...
accuracy(BC)=     length(find(predict_label == practise_label))/length(practise_label)*100;
BC=BC+1;
% ((framelenth-245)/49+1)
 end
% end
