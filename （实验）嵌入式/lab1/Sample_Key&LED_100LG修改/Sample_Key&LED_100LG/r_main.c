...
void main(void)
{
	...
	{
                R_WDT_Restart();
		Keyboard_scan();  					/*scan which button pressed*/
...
void dispaly(void)
{
	if(array_key[0]!=num_keyboard)//the second char
	{
		array_key[2]=array_key[1];
		array_key[1]=array_key[0];
		array_key[0]=num_keyboard;
	}
...
/*void dispaly(void)
{
	...
	if (count==1)
	{
	        array_key[1]=num_keyboard%10;    //the second char
	...
	if (count==2)
	{
	        array_key[2]=num_keyboard%10;    //the second char
